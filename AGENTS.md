...
- `OnPush` is NOT the default change detection strategy, even in Angular v22+. Always set `changeDetection: ChangeDetectionStrategy.OnPush` explicitly on every `@Component` — do not rely on it being implied.
...
- **Plurals use CLDR categories, not a single `{{count}}` string.** ngx-translate has no plural support, so `core/i18n/plural-compiler.ts` (`PluralCompiler`, wired via `provideTranslateCompiler` in `app.config.ts`) compiles any value written as `{ "one": ..., "other": ... }` into a function that picks the form with `Intl.PluralRules` for that file's language. Call sites stay `'key' | translate: { count: n }`. Arabic strings should cover all six categories (`zero`/`one`/`two`/`few`/`many`/`other`); English only needs `one`/`other`. A missing category falls back to `other`.
